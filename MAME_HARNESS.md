...
For a repeatable manual session, MAME input recording can be enabled through
the same wrapper and replayed later with the sidecar instrumentation intact:

```text
tools/mame-play-capture /tmp/stunrun-roms-system /tmp/stunrun-recorded \
  --frames 21600 --every 600 --record course-run --fresh-nvram --clean-cfg

tools/mame-play-capture /tmp/stunrun-roms-system /tmp/stunrun-replay \
  --frames 21600 --every 600 \
  --playback /tmp/stunrun-recorded/course-run.inp \
  --fresh-nvram --clean-cfg
```

Recording and playback are mutually exclusive. Playback ignores live input;
its `.inp` file must be generated by the same MAME revision, and a fresh
NVRAM/config pair should be used when comparing runs.
