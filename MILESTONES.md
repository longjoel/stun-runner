...
- [ ] **M1 — Evidence-backed machine and interconnect contract**: using the pinned ROM set and MAME runtime, produce a repeatable, provenance-backed map of every active processor, memory/device region, interrupt path, and inter-processor boundary needed for the first reconstruction slice. Promote only experimentally verified facts into machine-map metadata, checkpoint selectors, and harness capabilities; record unresolved semantics as bounded follow-up questions.
