...
- [x] **M0 — Reproducible machine, harness, evidence, and toolchain baseline**: complete Step 0 driver mining against the pinned MAME runtime identity; freeze one ROM set and MAME build; commit the MAME-derived hash manifest; validate local ROMs; reconcile runtime device tags with the mined machine map; generate stable static listings for every active programmable processor; establish the browser-automation-style MAME harness; run a deterministic bounded boot/title experiment; produce a canonical checkpoint and failure artifact bundle; prove bounded debugger tracing and trace-to-LCOV for at least one processor; and validate minimal code-emission paths for every active programmable CPU/DSP requiring replacement code. Environment provenance limitations are recorded in `analysis/m0-audit.md`.
- [x] **M1 — Evidence-backed machine and interconnect contract**: using the pinned ROM set and MAME runtime, produce a repeatable, provenance-backed map of every active processor, memory/device region, interrupt path, and inter-processor boundary needed for the first reconstruction slice. Promote only experimentally verified facts into machine-map metadata, checkpoint selectors, and harness capabilities; record unresolved semantics as bounded follow-up questions. Completion evidence is recorded in `analysis/interconnect.md`, `analysis/m1-landmarks.md`, and the linked experiment metadata.
- [x] **M2 — Reproduction image/toolchain integration**: use the already-proven emission paths to produce the smallest replacement/rebuilt image accepted by the original/emulated environment. M2 proves image layout, linking/placement, and runtime integration; it does not defer basic assembler/encoder feasibility from M0. The fixed-origin ADSP image proof is recorded in `reference/experiments/stunrun/m2-adsp-replacement-image.metadata.json`.
- [x] **M3 — Reset and initialization**: reproduction reaches the expected early initialization path and stable loop/checkpoint. The C-emitted ADSP init-state slice is installed before the original upload, reaches bounded loop `PC=0x0050`, reproduces the four DM landmarks with zero readback mismatches, and matches the synchronized non-ADSP checkpoint boundary in `reference/experiments/stunrun/m3-adsp-c-init-state-frame61.metadata.json`.
- [x] **M4 — Native shell**: Linux target has deterministic timing, input, rendering/logging scaffolding, harness-compatible replay/checkpoint support, and no speculative game logic. Evidence: `analysis/m4-native-shell.md`.
