"""Public ROM-free tests for the Agent 2 C port of the ADSP init slice.

Compiles reproduction/adsp/adsp_init_image.c with the system C compiler
...
class AdspCImageTests(unittest.TestCase):
    def test_c_self_check_passes(self):
        cc = shutil.which("cc") or shutil.which("gcc") or shutil.which("clang")
        self.assertIsNotNone(cc, "no C compiler available")
        with tempfile.TemporaryDirectory() as directory:
            binary = pathlib.Path(directory) / "adsp_init_image_test"
            compile_proc = subprocess.run(
                [cc, "-std=c99", "-Wall", "-Wextra", "-o", str(binary),
                 str(SRC / "adsp_init_image.c"),
                 str(SRC / "adsp_init_image_test.c")],
