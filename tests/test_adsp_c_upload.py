"""Public ROM-free tests for the Agent 2 C port of the ADSP upload slice.

Compiles reproduction/adsp/adsp_upload_stream.c with the system C compiler
...
class AdspCUploadTests(unittest.TestCase):
    def test_c_self_check_passes(self):
        cc = shutil.which("cc") or shutil.which("gcc") or shutil.which("clang")
        self.assertIsNotNone(cc, "no C compiler available")
        with tempfile.TemporaryDirectory() as directory:
            binary = pathlib.Path(directory) / "adsp_upload_stream_test"
            compile_proc = subprocess.run(
                [cc, "-std=c99", "-Wall", "-Wextra", "-o", str(binary),
                 str(SRC / "adsp_upload_stream.c"),
                 str(SRC / "adsp_upload_stream_test.c")],
