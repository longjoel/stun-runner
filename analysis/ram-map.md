...
| `0xFF9578–0xFF9579` | separate state/index word read by the object-award path at `0x03A2B2`, `0x03A2DA`, and `0x03A2F8`; a bounded human-play run observed progression `0→1→2→3→4→5`, with a transient `5→4→5` at the end | `DYNAMIC-PROGRESSION-CONFIRMED; DISPLAYED TRACK LABEL UNPROVEN` |
