...
| `0xFFFF8014` | position/progress candidate consumed by road, trajectory, geometry, and object-event paths; copied to `0xFFDCC0`, `0xFFDB36`, and `0xFFDDEC`, with shifted forms passed to placement helpers; unit and writer ownership remain unresolved | `STATIC-DATAFLOW + SNAPSHOT-CORRELATION; POSITION/PROGRESS-CANDIDATE` |
...
| `0xFF9578–0xFF9579` | separate state/index word read by the object-award path at `0x03A2B2`, `0x03A2DA`, and `0x03A2F8`; recorded human play observed `0→1→2→3→4→5`, with all five normal nonzero writes attributed to `0x0320BC`; terminal `5→4→5` is separately attributed to cleanup/re-entry writers `0x0291A2` and `0x0252EC` | `DYNAMIC-WRITER-CONFIRMED; TERMINAL-CLEANUP-CONFIRMED; DISPLAYED TRACK LABEL UNPROVEN` |
...
| `0xFFDCC6–0xFFDCC7` | 16-bit trajectory coordinate/state field; `0x0387C6`, `0x038D94`, and `0x038DF0` initialize it, `0x0388DE–0x038942` integrate and clamp it using the motion delta at `0xFFDD16`, and the effect/object builders consume it through the trajectory record at `0xFFDAF0`; the ROM bytes at `0x04D771`/`0x04D773` pin both arithmetic shifts to one bit; physical axis and effect identity remain unresolved | `STATIC-RESOLVED-TRAJECTORY; NOT-AMMO` |
